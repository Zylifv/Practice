I wanted to try and create my own version of Yahtzee which was inspired from Balatro.
I like the abilities presented in that game so that the player can munipulate outcomes with variance.
...
Things to do:
 ...
 - Eventually add different ways the player can change outcomes with card effects, such as:
        ...
        > Guaranteeing a dice rolls how you want (choose the number)
        > One less dice in a round gives currency? (Add option for currency like other games)
        > Betting on a certain score category awards a way to upgrade the score for that outcome? (Betting on a full-house, then rolling one upgrades its score by 0.1% etc.)
